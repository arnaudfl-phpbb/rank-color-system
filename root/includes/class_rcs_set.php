...
* @version $Id: class_rcs_set.php,v 0.2 15/03/2009 18:28 reddog Exp $
...
class rcs_set
{
	...
	*/
	function pagination($requester, $parms=array(), $total_items, $items_per_page, $start_item, $item_name_count='', $add_prevnext_text=true)
	{
		global $template, $lang;
		global $get;

		// init
		$parms = !is_array($parms) ? array() : $parms;
		$item = array(
		...
		$page_string = ($on_page == 1) ? '<strong>1</strong>' : '<a href="' . $get->url($requester, $parms, true) . '">1</a>';
		...
		if ( $total_pages > 5 )
		{
			...
			{
				$page_string .= ($i == $on_page) ? '<strong>' . $i . '</strong>' : '<a href="' . $get->url($requester, $parms + array('start' => (($i - 1) * $item['per'])), true) . '">' . $i . '</a>';
		...
		$page_string .= ($on_page == $total_pages) ? '<strong>' . $total_pages . '</strong>' : '<a href="' . $get->url($requester, $parms + array('start' => (($total_pages - 1) * $item['per'])), true) . '">' . $total_pages . '</a>';

		if ( $add_prevnext_text )
		{
			if ( $on_page != 1 ) 
			{
				$page_string = '<a href="' . $get->url($requester, $parms + array('start' => (($on_page - 2) * $item['per'])), true) . '">' . $lang['Previous'] . '</a>&nbsp;&nbsp;' . $page_string;
			}

			if ( $on_page != $total_pages )
			{
				$page_string .= '&nbsp;&nbsp;<a href="' . $get->url($requester, $parms + array('start' => ($on_page * $item['per'])), true) . '">' . $lang['Next'] . '</a>';
			}
		}
